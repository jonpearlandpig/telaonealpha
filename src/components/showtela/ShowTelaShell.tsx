            ...
            {vm.activeOps.map((p) => (
              <button key={p.id} onClick={() => setSheet({ type: 'person', name: p.name, role: p.latest })} className="flex items-center gap-3 py-3 text-left">
                <div className="relative">
                  ...
                  <button
                    onClick={(e) => { e.stopPropagation(); openVoice(p.name) }}
                    className="absolute -bottom-0.5 -right-0.5 flex h-5 w-5 items-center justify-center rounded-full border-2 border-[#F8F6F2] bg-[#141210]"
                  >
        ...
        <div className="px-5 pt-[58px]">
          <div className="flex flex-col items-center py-8">
            <div className="relative">
              ...
              <button
                onClick={() => openVoice(user?.name)}
                className="absolute -bottom-1 -right-1 flex h-7 w-7 items-center justify-center rounded-full border-2 border-[#F8F6F2] bg-[#141210] shadow-[0_2px_8px_rgba(0,0,0,0.30)]"
              >
...
cat > ~/telaonealpha/src/components/showtela/ShowTelaShell.tsx << 'ENDOFFILE'
'use client'
import { useEffect, useState } from 'react'
import { ActiveOpsRail } from './ActiveOpsRail'
import { BottomDock } from './BottomDock'
    ...
    <main className="relative mx-auto min-h-screen w-full max-w-sm bg-[#F8F6F2] pb-36 text-[#141210]">
      ...
      {showVoice && (
        <PearlDropVoice
          ...
          onClose={() => { setShowVoice(false); setTaggedPerson(undefined) }}
        />
      )}
