...
export function ShowTelaShell({ vm, onPearlDrop, isDemoMode = false }: { vm: ShowTelaViewModel; onPearlDrop: () => void; isDemoMode?: boolean }) {
  ...
  return (
    <main className='relative mx-auto min-h-screen w-full max-w-[430px] bg-[#F7F4EF] pb-32 text-[#111111]'>
      ...
      <ShowTelaHeader />
      {isDemoMode && !isEmpty ? <section className='mx-5 mt-3 rounded-2xl border border-amber-500/40 bg-amber-100/80 px-3 py-2 text-[11px] font-semibold tracking-[0.08em] text-amber-900'>DEMO MODE — LIVE CST MEMORY NOT CONNECTED</section> : null}
      {isEmpty ? (
        <OpeningSurface />
      ) : (
        <>
