...
export function ShowTelaShell({ vm, onPearlDrop, user }: { vm: ShowTelaViewModel; onPearlDrop: () => void; user?: { name: string; email: string; image: string } }) {
  ...
  const feed: ContinuityEvent[] = vm.feed.map((i) => ({ id: i.id, headline: i.title, body: i.summary, timestamp: i.timestamp, image: i.image, tags: i.linkedEntities ?? [], owner: { id: i.owner, name: i.owner }, isNew: i.unresolved, pressure: (i.pressure as 'low' | 'medium' | 'high' | undefined) ?? (i.unresolved ? 'high' : 'low') }))
