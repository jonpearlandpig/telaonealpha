...
export function ContinuitySheet({ open, personName, personRole, summary, rhythm, events, unresolved, operations }: { open: boolean; personName: string; personRole?: string; summary?: string; rhythm?: string; events: ContinuityEvent[]; unresolved: UnresolvedObject[]; operations: OperationEntity[] }) {
  if (!open) return null
  ...
  return (
    <section className='fixed inset-x-0 bottom-0 z-50 rounded-t-3xl border border-[#E5DED2] bg-white p-4 shadow-[0_-20px_40px_rgba(0,0,0,0.2)]'>
      ...
      <p className='text-xs text-[#6E604F]'>{personRole ?? 'Operator'} {rhythm ? `• ${rhythm}` : ''}</p>
