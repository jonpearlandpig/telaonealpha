...
const STATE_TONES: Record<OperationalCalendarEvent['pressureState'], { label: string; dot: string; text: string; bg: string }> = {
  ...
  pressure: { label: 'Needs A Check', dot: '#D68A3A', text: '#8F5520', bg: '#F8E6D3' },
  critical: { label: 'Attention Needed', dot: '#C85B4A', text: '#8E352C', bg: '#F7DDD8' },
  waiting: { label: 'Waiting', dot: '#8B847B', text: '#625B54', bg: '#ECE7DF' },
  needs_decision: { label: 'Needs Review', dot: '#B7772D', text: '#7A481C', bg: '#F5E2CD' },
  tela_ready: { label: 'Ready', dot: '#1F6B5F', text: '#174E46', bg: '#DCEDE8' },
}
    ...
    <article data-testid="calendar-event-card" className="rounded-[22px] border border-[#E8DDCC] bg-[#FFFDF8] px-4 py-4 shadow-[0_10px_28px_rgba(27,22,16,0.06)]">
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        <div className="rounded-[14px] bg-[#F6F0E7] px-3 py-2">
          ...
          <p className="mt-1 line-clamp-1 text-[12px] font-medium text-[#29231C]">{people || 'Open assignment'}</p>
	        ...
	        <div className="rounded-[14px] bg-[#F6F0E7] px-3 py-2">
	          <p className="text-[9px] font-semibold uppercase tracking-[0.14em] text-[#9A825F]">Area</p>
	          <p className="mt-1 line-clamp-1 text-[12px] font-medium text-[#29231C]">{departments || 'General'}</p>
        </div>
        <div className="rounded-[14px] bg-[#F6F0E7] px-3 py-2">
          <p className="text-[9px] font-semibold uppercase tracking-[0.14em] text-[#9A825F]">Source</p>
	          <p className="mt-1 line-clamp-1 text-[12px] font-medium text-[#29231C]">{event.sourceArtifactTitle || 'Uploaded source'}</p>
        </div>
        <div className="rounded-[14px] bg-[#F6F0E7] px-3 py-2">
	          <p className="text-[9px] font-semibold uppercase tracking-[0.14em] text-[#9A825F]">Last Update</p>
          <p className="mt-1 line-clamp-1 text-[12px] font-medium text-[#29231C]">{shortDateTime(event.freshnessTimestamp ?? event.importedAt) || 'Pending'}</p>
        </div>
      </div>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        {event.location && (
          <span className="rounded-full bg-[#EFE8DA] px-2.5 py-1 text-[11px] font-medium text-[#6B5D4B]">{event.location}</span>
	        ...
	        <span className="rounded-full bg-[#EFE8DA] px-2.5 py-1 text-[11px] font-medium text-[#6B5D4B]">{event.continuityState} update state</span>
      ...
      {event.telaHint && (
        <div className="mt-3 rounded-[16px] border border-[#E3D3B8] bg-[#FBF5EA] px-3 py-2">
	          <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[#9A7C46]">TELA Note</p>
