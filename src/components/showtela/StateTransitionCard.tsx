  ...
  if (!transition) {
    return (
      <section className="rounded-[22px] border border-dashed border-[#D6C9B7] px-4 py-5 text-center">
        <p className="text-[13px] font-semibold text-[#6B5D4B]">No state change yet.</p>
        <p className="mt-1 text-[12px] leading-relaxed text-[#8B847B]">Changes appear when pressure or saved updates move.</p>
      </section>
    )
  }
  ...
  return (
    <section className="rounded-[22px] border border-[#E5D8C7] bg-[#FFFDF8] px-4 py-4 shadow-[0_10px_26px_rgba(27,22,16,0.05)]">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#9A7C46]">State Change</p>
      ...
      <div className="mt-3 rounded-[16px] border border-[#E3D3B8] bg-[#FBF5EA] px-3 py-2">
        <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[#9A7C46]">Update Shift</p>
        <p className="mt-1 text-[12px] leading-relaxed text-[#5B4D3D]">{transition.continuityShift} saved movement / {transition.eventId}</p>
      </div>
    </section>
  )
}
