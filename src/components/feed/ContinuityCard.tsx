...
export function ContinuityCard({ card }: { card: Card }) {
  ...
  return <article className='bg-white rounded-[30px] px-5 py-5 space-y-4 border border-[#EFE9DE] shadow-[0_10px_28px_rgba(20,20,20,0.06)]'>
    <div className='flex justify-between items-start gap-3'>
      <div className='flex items-start gap-3 min-w-0'>
        ...
        <div className='min-w-0'>
          <Link href={`/entity/${encodeURIComponent(card.owner)}`} className='text-[11px] leading-4 text-[var(--text-secondary)] truncate block'>{card.owner} · {card.department}</Link>
          <h3 className='text-[19px] leading-[1.24] mt-1.5 tracking-[-0.012em] text-[var(--text-primary)]'>{card.headline}</h3>
        </div>
      </div>
      <p className='text-[10px] leading-4 text-[var(--text-tertiary)] shrink-0'>{card.timestamp}</p>
    </div>

    <p className='text-[14px] leading-[1.58] text-[var(--text-secondary)] line-clamp-3'>{card.summary}</p>
    ...
    <div className='grid grid-cols-4 gap-2 pt-1 text-[11px]'>
      {['Acknowledge', 'Add Note', 'Resolve', 'Pin'].map((a) => <button key={a} className='min-h-10 rounded-full border border-[var(--border)] bg-white text-[var(--text-secondary)] active:translate-y-[1px]'>{a}</button>)}
    </div>
  </article>
}
