...
export function ArtifactRenderer({ artifact }: { artifact: ArtifactRecord }) {
  if (artifact.previewUrl) {
    return <iframe title={artifact.title} src={artifact.previewUrl} style={{ width: '100%', height: 260, border: 0, borderRadius: 10, background: '#081321' }} />
  }
  if (artifact.html) {
    return <iframe title={artifact.title} srcDoc={artifact.html} style={{ width: '100%', height: 260, border: 0, borderRadius: 10, background: '#081321' }} sandbox="allow-scripts" />
  }
