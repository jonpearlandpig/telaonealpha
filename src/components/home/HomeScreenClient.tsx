...
export function HomeScreenClient({ initialCards, initialOps }: Props) {
  ...
  return <div className='min-h-screen bg-[var(--bg)] text-[var(--text-primary)]'>
    <div className='mx-auto max-w-[460px] pb-[124px]'>
      <HomeHeader />
      ...
      <main className='px-6 pt-5 space-y-8 transition-opacity duration-300'>
        ...
        {grouped.map((section) => section.items.length > 0 && (
          <section key={section.label} className='space-y-3'>
            <div className='flex items-end justify-between'>
              <h2 className='text-[11px] uppercase tracking-[0.18em] text-[var(--text-tertiary)]'>{section.label}</h2>
              <p className='text-[11px] text-[var(--text-secondary)]'>{section.hint}</p>
            </div>
