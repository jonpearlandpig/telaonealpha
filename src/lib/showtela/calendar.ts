...
export function buildOperationalCalendarEvents({
  ...
  const feedEvents = feed
    ...
    .map<OperationalCalendarEvent>((item) => {
      ...
      return {
        ...
        telaHint: unresolvedCount > 0 ? 'TELA can explain who is waiting on this and what it could affect next.' : 'TELA can give a quick read on why this matters today.',
  ...
  const timelineEvents = runtimeTimeline
    ...
    .map<OperationalCalendarEvent>((item) => {
      ...
      return {
        ...
        continuityState: deriveContinuityState(item.timestamp, anchorDate),
        density: 'light',
        telaHint: 'TELA can explain what shifted and what it may affect next.',
  ...
  const unresolvedEvents = unresolvedItems.map<OperationalCalendarEvent>((item, index) => {
    ...
    return {
      ...
      summary: item.operation ? `${item.operation} is waiting on a final detail.` : 'This still needs an owner and a follow-up.',
      unresolvedCount: 1,
      pressureState: status,
      continuityState: typeof item.aging === 'number' ? (item.aging >= 4 ? 'stale' : item.aging >= 2 ? 'aging' : 'fresh') : 'unknown',
      density: 'light',
      telaHint: item.blocking ? 'TELA can explain who is blocked and what slips if this stays open.' : 'TELA can suggest the next follow-up.',
  ...
  const operationEvents = operations
    ...
    .map<OperationalCalendarEvent>((item, index) => {
      ...
      return {
        ...
        summary: item.latest || `${title} is in view for the day.`,
        unresolvedCount,
        pressureState: status,
        continuityState: item.latest ? 'fresh' : 'unknown',
        density: deriveDensity(unresolvedCount),
        telaHint: unresolvedCount > 0 ? 'TELA can explain what this is waiting on.' : 'TELA can keep this close without crowding the day.',
