...
function buildFocusItem(state: OperationalStateRecord, nextShowDate: Date | null): FocusItem {
  const impact = mapSeverityToImpact(state.severity)
  const pressure = computePressureFromState(state)
  const timeSensitivity = computeTimeSensitivity(nextShowDate)
...
export function buildFocusEngine(
  ...
  const allFocusItems = candidates
    .map((state) => buildFocusItem(state, nextShowDate))
