...
export default async function SignInPage() {
  ...
  const baseUrl = process.env.NEXTAUTH_URL ?? 'https://telaonealpha-jn9i.vercel.app'
  const redirectUri = `${baseUrl}/api/auth/callback`

  const googleAuthUrl = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${encodeURIComponent(clientId)}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&scope=openid%20email%20profile&access_type=offline`
    ...
    <main className="flex min-h-screen flex-col items-center justify-center bg-[#F8F6F2] px-6">
      <div className="w-full max-w-[340px]">
        ...
        <div className="rounded-[24px] border border-[#EAE4DA] bg-white px-6 py-8 shadow-[0_8px_32px_rgba(17,17,17,0.08)]">
          ...
          <a
            href={googleAuthUrl}
            className="mt-6 flex w-full items-center justify-center gap-3 rounded-[14px] border border-[#EAE4DA] bg-white px-4 py-3.5 shadow-[0_2px_8px_rgba(0,0,0,0.06)]"
          >
